...
class LatestData:
    ...
    def get_station_names_bw_dists_of_cardinal_points(
        ...
        def calc_coords(lat, long, min_dist):
            ...
            while haversine([lat, long], [north_lat, long]) < min_dist:
                north_lat += degree_step
                if north_lat == 90 + degree_step:
                    break
            while haversine([lat, long], [south_lat, long]) < min_dist:
                south_lat -= degree_step
                if south_lat == -(90 + degree_step):
                    break
            
            west_long, east_long = long, long
            while haversine([lat, long], [lat, west_long]) < min_dist:
                west_long -= degree_step
                if west_long == -(180 + degree_step):
                    west_long = 180 - degree_step
            while haversine([lat, long], [lat, east_long]) < min_dist:
                east_long += degree_step
                if east_long == 180 + degree_step:
                    east_long = -180 + degree_step
        ...
        extreme_points = {}
        for dist in dists:
            extreme_points[dist] = calc_coords(lat, long, min_dist=dist)
