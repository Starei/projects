...
class UserController extends Controller
{
    ...
    public function usersListing() {
        $users       =       User::all();
        if(count($users) > 0) {
            return response()->json(["status" => $this->status_code, "success" => true, "count" => count($users), "data" => $users]);
        }
        else {
    ...
    public function userDelete($id) {
        ...
        if(!is_null($user)) {
            ...
            if($delete_status == 1) {
                return response()->json(["status" => $this->status_code, "success" => true, "message" => "user record deleted successfully"]);
            }
            else {
